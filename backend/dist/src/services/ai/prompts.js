...
exports.buildInvestigationPrompt = buildInvestigationPrompt;
exports.formatEvidenceForDisplay = formatEvidenceForDisplay;
// ============================================================
// PROMPT TEMPLATES
// ============================================================
exports.PROMPT_VERSION = 'v1.0.0';
function buildTriagePrompt(title, description) {
...
function buildInvestigationPrompt(incident, ragResult) {
    const { similarIncidents, knowledgeArticles, evidenceStrength } = ragResult;
    const resolvedIncidentsSection = similarIncidents.length > 0
        ? similarIncidents
            .map((inc, i) => `
...
Similarity Score: ${(inc.similarity * 100).toFixed(1)}%`)
    ...
    const knowledgeSection = knowledgeArticles.length > 0
        ? knowledgeArticles
            .map((art, i) => `
...
- Maximum Similarity: ${(evidenceStrength.max_similarity * 100).toFixed(1)}%`;
    const hallucGuard = evidenceStrength.label === 'low'
        ? `
...
function formatEvidenceForDisplay(analysis, ragResult) {
