...
exports.calculateEvidenceStrength = calculateEvidenceStrength;
const THRESHOLDS = {
    STRONG: 0.85,
    USEFUL: 0.75,
    MODERATE: 0.60,
 ...
 */
function calculateEvidenceStrength(resolvedIncidents, knowledgeArticles) {
    const maxSimilarity = Math.max(...resolvedIncidents.map((r) => r.similarity), ...knowledgeArticles.map((a) => a.similarity), 0);
    ...
    const usefulResolved = resolvedIncidents.filter((r) => r.similarity >= THRESHOLDS.USEFUL).length;
    const resolvedScore = Math.min(30, usefulResolved * 15);
    // Factor 3: Relevant knowledge articles (20 points max)
    const relevantArticles = knowledgeArticles.filter((a) => a.similarity >= THRESHOLDS.MODERATE).length;
...
//# sourceMappingURL=evidenceScorer.js.map